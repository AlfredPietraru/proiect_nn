...
def validate_semi_supervised(
    ...
    for step_idx, (images, targets) in enumerate(tqdm(dt_test, desc="Validation")):
        ...
        loss_sum += sum(loss_dict.values()).item()
        student.eval()
...
def pipeline_semi_supervised(
    ...
    student = build_model(cfg=cfg).to(cfg.train.device)
    student, _, _ = load_checkpoint(path=checkpoint_path, model=student, optimizer=None, device=cfg.train.device)
    ...
    for epoch in tqdm(range(cfg.train.epochs), desc="Semi-Supervised Epochs"):
        sup_hist, unsup_hist, train_loss = train_semi_supervised_one_epoch(
            ...
            data=data, device=cfg.train.device, lambda_unsup=cfg.ssl.unsup_weight,
            score_thr=cfg.ssl.pseudo_conf_thr, nms_iou=0.5, metric_sup=metric_sup, metric_unsup=metric_unsup)

        val_hist, val_loss = validate_semi_supervised(
            student=student, dt_test=data["test"], device=cfg.train.device,
